...
export default function PaymentStripeForm({ clientSecret,object,setActiveIndex,setPaymentDialogVisibility}) {
    ...
    const handleSubmit = async (event) => { 
        ...
        if (error) { 
                  ...
                  let planCharges=""
                   if(object.plan  === simpricing._id){  
                       ...
                       for(let i=0;i<simpricing.additionalFeature.length;i++){ 
                        ...
                        if(simpricing.additionalFeature[i].featureName._id === simpricing.additionalFeature[i]._id  ){let obj={ 
                    ...
                    for(let i=0;i<devicepricing.selectdiscount.length;i++){ 
                      ...
                      for(let i=0;i<devicepricing.additionalFeature.length;i++){ 
                        if(deviceadditional[k] === devicepricing.additionalFeature[i]._id){let obj={ 
            ...
            Axios.post(`${BASE_URL}/api/web/invoices/generateInvoice`,dataToSend).then((response)=>{ 
               ...
               const parseLoginRes = JSON.parse(loginRes);        
                ...
                priority: "highest",
                  ...
                  Axios.post(`${BASE_URL}/api/web/notes/`, data).then(()=>{   
