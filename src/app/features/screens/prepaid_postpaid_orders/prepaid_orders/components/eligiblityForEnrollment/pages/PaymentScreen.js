    ...
    return (
        <form onSubmit={formik.handleSubmit}>
            ...
            <div className="flex flex-wrap flex-row justify-content-around">   
                  ...
                  <Button label="Back" type="button" onClick={()=>{  
                       if(currentScreen !== 1){
                    ...
                    setActiveIndex(1);
                     ...
                     <Button label="Continue" type={currentScreen < 4 ? "button":"submit"}  icon={isLoading ? "pi pi-spinner":""} disabled={(current === "" && currentScreen === 1)||(currentPlanSelect === "" && currentScreen === 2)} onClick={()=>{ 
                           if(currentScreen !== 3){
                        setCurrentScreen(prev=>prev=prev+1)  
                           }       
                        else{ 
                                    ...
                                    JSON.parse(localStorage.getItem("inventoryType"))?.map(item=>{   
                                         ...
                                         return( 
                                           ...
                                           <img src={`./${item.label}.jpg`}/>
                                             <button type="button"  disabled={item?.label  === current || paymentInfo} onClick={()=>{ 
...
}      
{
                            ...
                            { 
                                    JSON.parse(localStorage.getItem("planprices"))?.map(item=>{   
                                         ...
                                         return( 
                                            ...
                                            <h1 className="planprice"><span>$</span><span>{item?.price}</span></h1> 
                                             <p className="chargetype">Monthly</p>      
                                              <p className="voiceallowance">Voice Allowance  {item?.voiceAllowance} <span>{item.voiceAllowanceUnit}</span></p>
                                              <p className="dataallowance">Data Allowance  {item?.dataAllowance} <span>{item.dataAllowanceUnit}</span></p>
                                              <p className="textallowance">Text Allowance  {item?.textAllowance} <span>{item.textAllowanceUnit}</span></p>
                                            
                                             <button type="button" disabled={item?._id  === currentPlanSelect || paymentInfo} onClick={()=>{ 
                                              onPlanSelect(item)
                                             }}>Select</button>      
                                            </div>  
                                            </>:undefined
    ...
    {currentScreen === 3 ? <div className="w-full flex flex-wrap flex-row justify-content-left w-full">
