...
const PrepaidRejectedEnrollments = () => {
   ...
   const [visible,setVisible]=useState(false)
    ...
    const getAllEnrollments = async () => {
        setIsLoading(true);
        try {
            const res = await Axios.get(`${BASE_URL}/api/user/rejectedEnrollmentUser?userId=${parseLoginRes?._id}&accountType=Prepaid`);
    ...
    const viewRow = async (rowData) => {
        ...
        try {
            ...
            if (response?.status === 201 || response?.status === 200) {
                ...
                let storedData = JSON.parse(localStorage.getItem("comingfo")) || {};
                if (storedData) {
                    storedData = false;
                    localStorage.setItem("comingfromincomplete", JSON.stringify(storedData));
                } else {
                    storedData = false;
                    localStorage.setItem("comingfromincomplete", JSON.stringify(storedData));
                }
...
export default PrepaidRejectedEnrollments;
