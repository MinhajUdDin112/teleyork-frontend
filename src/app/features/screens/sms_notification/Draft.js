...
const Draft = () => {
    ...
    const getAllDraft = async () => {
        try {
            const response = await Axios.get("http://dev-api-ijwireless.teleyork.com/api/sms/draft");
    ...
    const handleSubmit = () => {
        const response = Axios.post("http://dev-api-ijwireless.teleyork.com/api/sms/send/64ad9b07fc04dc6ca623b9c3")
        if (response.status === 200) {
