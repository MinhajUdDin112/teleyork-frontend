...
export default function DepartmentWiseAgentRejectedDateRangeChart({role,roleId,BASE_URL,startDate,endDate}){   
            ...
            let endDateEnrollment = endDate;
              ...
              if(role === "CSR" || role === "TEAM LEAD"){
                  qaRejectedEnrollment= response.data.data.filter((enrollment) => {   
                    return DateTime.fromFormat(enrollment.createdAt, "d LLL yyyy, h:mm a", { zone: "America/New_York" }).toSeconds() >= startDate && DateTime.fromFormat(enrollment.createdAt, "d LLL yyyy, h:mm a", { zone: "America/New_York" }).toSeconds() <=endDateEnrollment && enrollment.rejectedBy.department.department === "QA"
       
                  });   
                  provisioningRejectedEnrollments= response.data.data.filter((enrollment) => {   
                    return DateTime.fromFormat(enrollment.createdAt, "d LLL yyyy, h:mm a", { zone: "America/New_York" }).toSeconds() >= startDate && DateTime.fromFormat(enrollment.createdAt, "d LLL yyyy, h:mm a", { zone: "America/New_York" }).toSeconds() <=endDateEnrollment && enrollment.rejectedBy.department.department === "PROVISIONING"
