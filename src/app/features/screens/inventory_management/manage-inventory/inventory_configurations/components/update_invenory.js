...
export default function UpdateInventory({data ,setRefresh ,setEditInventoryVisibility}){ 
    const toast = useRef(null);
    const formik=useFormik({ 
        initialValues:{ 
         inventoryType:data?.inventoryType, 
         serviceProvider:parseLoginRes?.company, 
         inventoryTypeId:data?._id
        }, 
        validationSchema, 
        onSubmit:(values)=>{ 
       Axios.put(`${BASE_URL}/api/inventoryType/update`,formik.values).then(()=>{
        toast.current.show({ severity: "success", summary: "Inventory Type Updation", detail: "Updated Successfully" });
        setTimeout(() => {
